// components/SeoAssistant.tsx
"use client";
import { useMemo } from "react";
import {
  firstParagraph, countWords, keywordDensity,
  countLinksMd, countImagesMd, countHeadings, gulpeaseIndex
} from "@/lib/seo";
...
export default function SeoAssistant({
  ...
  const data = useMemo(() => {
    ...
    const inputsEmpty =
      !t && !s && !md.trim() && !st && !sd && !kw;

    // Se tutto è vuoto: 0% e nessuna spunta
    if (inputsEmpty) {
      const emptyChecks = [
        "Titolo presente",
        "SEO Title 35–60 caratteri",
        "Meta Description 80–160 caratteri",
        "Keyword focus impostata",
        "Keyword nel titolo",
        "Keyword nello slug",
        "Keyword nel primo paragrafo",
        "Lunghezza contenuto ≥ 300 parole",
        "Densità keyword 0.5–2.5%",
        "Almeno 2 H2",
        "Struttura con sottosezioni (H3 o più H2)",
        "≥ 1 link interno",
        "≥ 1 link esterno",
        "≥ 1 immagine",
        "Alt per tutte le immagini",
        "Almeno un alt con keyword",
        "Leggibilità (Gulpease) ≥ 40",
      ].map(label => ({ ok: false, label }));
    ...
    const checks = [
      ...
      // Keyword: OK solo se kw presente
      ...
      // Densità: valutata solo con kw e testo presenti
      { ok: !!kw && words > 0 && dens.density >= 0.5 && dens.density <= 2.5, label: `Densità keyword 0.5–2.5% (attuale ${dens.density.toFixed(2)}%)` },
      ...
      // Alt per tutte le immagini: valido solo se ci sono immagini
      { ok: imgs.count > 0 && imgs.withAlt >= imgs.count, label: "Alt per tutte le immagini" },
      // Alt con keyword: valido solo se kw presente e ci sono immagini
      { ok: !!kw && imgs.altWithKw >= 1, label: "Almeno un alt con keyword" },

      // Gulpease considerato solo se c'è contenuto
    ...
    return {
      checks, score, words,
      dens: { hits: dens.hits, density: dens.density },
      links, imgs, heads, gul
    };
  }, [title, slug, contentMd, seoTitle, seoDescription, focusKw]);
  ...
  return (
    <div className="border rounded-2xl p-4 bg-white grid gap-3">
      ...
      <div className="text-sm grid gap-1">
        <p>
          Parole: <strong>{data.words}</strong> ·
          Densità KW: <strong>{isFinite(data.dens.density) ? data.dens.density.toFixed(2) : 0}%</strong> ·
          Gulpease: <strong>{data.gul}</strong>
        </p>
      </div>
